  ...
  return (
    <div className="px-5 pb-28 sm:px-8">
      ...
      <p className="mt-2 text-[13px] text-muted-foreground">
        ...
        {post.dueAt && (
          <>
            {" · "}
            <span className={cn(
              isPastDue(post.dueAt) ? "text-destructive font-medium" : isDueSoon(post.dueAt) ? "text-primary font-medium" : "",
            )}>
      ...
      <article className="mt-6">
        <div className="whitespace-pre-wrap text-[16px] leading-[1.7] text-foreground/90">
          {post.body}
        </div>
      </article>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1 text-left">
                    <p className="truncate text-[14px] font-medium">{getFileName(att)}</p>
                    <p className="text-[12px] text-muted-foreground">{formatFileSize(att.size)}</p>
                  </div>
                ...
                <div className="space-y-1.5">
                  {subAttachments.map((att) => (
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1 truncate text-left">{getFileName(att)}</span>
        ...
        <div className="space-y-5 p-5">
          ...
          <div className="space-y-2">
            {submitFiles.map((file, i) => (
              <div key={i} className="flex items-center gap-2 rounded-xl bg-muted/30 px-3 py-2 text-[13px]">
                <Paperclip className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                <span className="min-w-0 flex-1 truncate">{file.name}</span>
                <span className="shrink-0 text-[12px] text-muted-foreground">{(file.size / 1024).toFixed(0)}KB</span>
          ...
          <button
            ...
            className={cn(
              ...
              (submitting || (!submitText.trim() && submitFiles.length === 0)) && "opacity-50 pointer-events-none",
            )}
